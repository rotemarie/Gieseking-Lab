...
data = []
input_file_name = input("Enter the excel file name: ")
with open(input_file_name, newline='') as f:
    for row in csv.reader(f):
        data.append(row[0])

for d in data:
    if d[4] == '-': #negative charge
        filename = d[:8]
        mode = d[9]
        if len(d) == 15: #negative displacement, float
         disp = d[-4:]
        elif len(d) == 14: #positive displacement, float
         disp = d[-3:]
        elif len(d) == 13: #negative displacement, integer
         disp = d[-2:]
        else: #positive displacement, integer
         disp = d[-1:]
    else: #positive charge
        filename = d[:7]
        mode = d[8]
        if len(d) == 14: #negative displacement, float
         disp = d[-4:]
        elif len(d) == 13: #positive displacement, float
         disp = d[-3:]
        elif len(d) == 12: #negative displacement, integer
         disp = d[-2:]
        else: #positive displacement, integer
         disp = d[-1:]
